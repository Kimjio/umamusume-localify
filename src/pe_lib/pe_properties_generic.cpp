...
void pe_properties_generic<PEClassType>::create_pe(uint32_t section_alignment, uint16_t subsystem)
{
    ...
    nt_headers_.OptionalHeader.MajorLinkerVersion = 0xB;
...
//Sets linker version
template<typename PEClassType>
void pe_properties_generic<PEClassType>::set_linker_version( uint8_t version )
{
    nt_headers_.OptionalHeader.MajorLinkerVersion = version;
}
